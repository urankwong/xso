...
1. 首次启动会自动导入内置源（6 个磁力搜索源 + 5 个网盘聚合源 + 音乐/书籍/有声源），也可在「源」页点右上角重新导入
2. 「源」页点任一源进入**源调试器**，输入关键词试搜，能看到每步结果与错误——用于校准真实站点规则
3. 回「搜索」页输入关键词 → 各源并发搜索、流式出结果（复制/打开/收藏）
4. 「收藏」页可批量活性检测，失效标红；「历史」页点击关键词回搜

> **关于内置源**：App 遵循"空壳原则"，预装的磁力和网盘源使用公开的搜索引擎/聚合站规则（CSS 选择器 + 两段式详情解析）。站点改版是常态——若某源失效，可在「源」页长按删除后通过社区获取最新规则或自行改写；也可导入 MusicFree / 洛雪 / Legado 格式的第三方源。
